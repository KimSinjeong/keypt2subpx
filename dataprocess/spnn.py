...
parser.add_argument('--conf_thresh', '-ct', type=float, default=0.00015, 
...
out_dir = 'data/megadepth/' + opt.variant + '_spnn/'
...
with torch.no_grad():
	for i, data in enumerate(tqdm(dataloader)):
		...
		ratios = pred['similarity'][0][idx[:,0],idx[:,1]] # N

		# Patches start
		bias = torch.tensor([[opt.patch_radius]*2], device=pts1.device)
		pad_amount = opt.patch_radius - opt.nms_dist
		...
		score1_padded = torch.nn.functional.pad(pred['keypoint_scores0'][:, :img1_shape[0], :img1_shape[1]], [pad_amount]*4, mode='constant', value=0.)
		scorepatch1 = extract_patches(score1_padded, (pts1 - bias + pad_amount).int(), 2*opt.patch_radius+1)[0]
		score2_padded = torch.nn.functional.pad(pred['keypoint_scores1'][:, :img2_shape[0], :img2_shape[1]], [pad_amount]*4, mode='constant', value=0.)
